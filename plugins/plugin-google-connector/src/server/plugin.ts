...
export class PluginNocoToolsServer extends Plugin {
    ...
    this.app.resourceManager.registerActionHandlers({
      ...
      'googleConnections:callback': async (ctx, next) => {
        ...
        try {
          ...
          this.app.logger?.warn?.(`[google-connector] OAuth callback error: ${err?.message || err}`);
  ...
  private async wipeAllConnections(reason: string) {
    try {
      ...
      this.app.logger?.info?.(`[google-connector] Cleared ${rows.length} Google connection(s) on ${reason}.`);
    } catch (err: any) {
      this.app.logger?.warn?.(`[google-connector] Failed to clear connections on ${reason}: ${err?.message || err}`);
    }
  }
}
