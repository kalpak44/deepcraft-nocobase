/**
 * The HTML served by `googleConnections:callback` — the page Google redirects
 * the OAuth popup to.
 *
...
<script>
  try {
    if (window.opener) {
      ...
      // nothing. With '*' the payload — which carries the user's Google email
      // address — would be readable by whatever document happened to be the
      // opener, including a hostile page that opened the app in a popup.
      //
      ...
      // its popup-closed path; googleConnections:status is re-fetched either
      // way, so the connection state stays correct. Losing a toast is the
      // right failure mode for a cross-origin mismatch.
      window.opener.postMessage(${jsonForScript({ source: 'nocobase-google-oauth', ...payload })}, window.location.origin);
    }
  } catch (e) {}
 ...
 * the payload into markup. `googleEmail` comes from Google's userinfo response
 * rather than from this codebase, so it is not ours to trust. U+2028/U+2029 are
 * also escaped — they are valid JSON but illegal raw in a JavaScript string
 * literal, and would otherwise be a syntax error rather than a vulnerability.
 */
