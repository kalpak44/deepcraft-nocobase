...
function getKey(): Buffer {
  // The literal is a last resort for a misconfigured app, never a real key.
  // Changing it makes tokens encrypted under it undecryptable — which only
  // affects an installation that ran with neither variable set.
  const material =
    process.env.APP_KEY ||
    process.env.NOCOBASE_APP_KEY ||
    'nocobase-google-connector-fallback-key-please-set-APP_KEY';
