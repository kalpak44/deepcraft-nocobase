...
/**
 * Register plugin-google-connector's Google functions as AI-callable tools.
 *
...
export function registerAITools(app: Application): void {
  ...
  if (!toolsManager || typeof toolsManager.registerTools !== 'function') {
    app.logger?.info?.('[google-connector] plugin-ai not detected; skipping AI tool registration. REST endpoints remain available.');
    return;
  }
  ...
  try {
    ...
    app.logger?.info?.(`[google-connector] Registered ${tools.length} AI tools (scope=GENERAL, from=loader).`);
  } catch (err: any) {
    app.logger?.warn?.(`[google-connector] Failed to register AI tools: ${err?.message || err}`);
  }
}
