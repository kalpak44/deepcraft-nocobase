...
export async function getConnection(app: Application, userId: number | string): Promise<StoredConnection | null> {
  const repo = app.db.getRepository('msConnections');
...
export async function ensureFreshAccessToken(
  ...
  if (!conn) throw new Error('Microsoft account not connected for this user.');
  ...
  try {
    ...
    const updated = await saveConnection(app, userId, {
      ...
      // Microsoft rotates refresh tokens on every use — always persist the
      // new one when present, unlike Google which normally omits it.
